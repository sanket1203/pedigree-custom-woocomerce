...
class WP_Users_List_Table extends WP_List_Table {
	...
	public function single_row( $user_object, $style = '', $role = '', $numposts = 0 ) {
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			$ja_disable_user_s = get_user_meta($user_object->ID, 'ja_disable_user',false);
			
			$status_active_v = $ja_disable_user_s[0];
			
			if($status_active_v==1){
			...
			if ( 'cb' === $column_name ) {
				...
				switch ( $column_name ) {
					...
					case 'name':
                        $r .= "
						<span id='success_msg$user_object->ID' style='color:red'></span></br>
						<input type='hidden' name='hidden_id' id='hidden_id$user_object->ID' value='$user_object->ID'>
						<input type='checkbox' name='ja_disable_user' id='ja_disable_user$user_object->ID' value='1' $status_active >
						<span class='description'>If checked, the user cannot login with this account.</span>";
						// $r .= "$user_object->first_name $user_object->last_name";
						break;
					case 'email':
						$r .= "<a href='" . esc_url( "mailto:$email" ) . "' id='o2'>$email</a>";
						break;
					case 'role':
						$r .= $role_name;
						break;
...
$(document).ready(function(){
	$("#ja_disable_user<?php echo $user_object->ID; ?>").click(function(){
		
		var hidden_id_val = $("#hidden_id<?php echo $user_object->ID; ?>").val();
		
		if($("#ja_disable_user<?php echo $user_object->ID; ?>").prop('checked') == true){
			
			var checkboxval=1;
          }
		if($("#ja_disable_user<?php echo $user_object->ID; ?>").prop('checked') == false){
	 ...
	 var checkboxval=0;
			...
			url: "<?php echo site_url(); ?>/wp-admin/users_ajax.php",
			data: {checkboxval:checkboxval,hidden_id_val:hidden_id_val},
			cache: false,
			success: function(data)
			{
				
				 $("#success_msg<?php echo $user_object->ID; ?>").text(data);
