...
jQuery(document).ready(function($) {
	...
	setTimeout(function(){
		$("video.wp-video-shortcode").each(function() {
			this.volume = $(this).closest('.item').find('.volume').val()/100;
		});
	}, 100);
	$(document).on('click', '.mejs-mute button', function(){
		var input = $(this).closest('.item').find('.volume');
		input.val(0);
	});
	$(document).on('click', '.mejs-unmute button', function(){
		var input = $(this).closest('.item').find('.volume');
		var volume_slider = $(this).closest('.item').find('.mejs-volume-slider');
		input.val(volume_slider.attr('aria-valuenow'));
	});
	$(document).on('click', '.mejs-volume-slider',function(){
		var volume_slider = $(this);
		var input = $(this).closest('.item').find('.volume');
		input.val(volume_slider.attr('aria-valuenow'));
		var out_event = 0;
