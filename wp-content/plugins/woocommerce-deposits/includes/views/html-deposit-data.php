		...
		<p class="form-field _wc_deposit_payment_plans_field">
			...
			if ( ! $plan_ids ) {
				...
				if ( ! empty ( $default_payment_plans ) ) {
					...
					echo '<br /><br />' . sprintf( esc_html__( 'The following plans will be used if no payment plan is selected: %s.', 'woocommerce-deposits' ), '<em>' . $default_payment_plan_string . '</em>' );
				}
			} ?>
		</p>
	</div>
