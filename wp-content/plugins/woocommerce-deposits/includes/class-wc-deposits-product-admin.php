...
class WC_Deposits_Plans_Product_Admin {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_email_order_details', array( $this, 'add_pay_link' ), 1, 1 );
	}
	...
	/**
	 * Add pay order link to remaining invoice email.
	 *
	 * @since 1.2.0
	 * @version 1.2.0
	 * @param object $order
	 */
	public function add_pay_link( $order ) {
		if ( $order->has_status( 'pending-deposit' ) ) : ?>
			<p><?php printf( __( 'An order has been created for you on %s. To pay for this order please use the following link: %s', 'woocommerce-deposits' ), get_bloginfo( 'name', 'display' ), '<a href="' . esc_url( $order->get_checkout_payment_url() ) . '">' . __( 'pay', 'woocommerce-deposits' ) . '</a>' ); ?></p>
		<?php endif;
	}
}
