...
class WC_Deposits_Scheduled_Order_Manager {
	...
	public static function schedule_orders_for_plan( $payment_plan, $original_order_id, $item ) {
		...
		$percent_remaining = 100 - $first_payment->amount; // this is a percent, e.g. 100 - 25 = 75
		...
		foreach ( $schedule as $schedule_row ) {
			...
			$row_discount = round( ( $item['deposit_deferred_discount'] / $percent_remaining ) * $schedule_row->amount, 2 );
	...
	public static function invoice_scheduled_orders() {
		...
		if ( $due_orders ) {
			...
			remove_filter( 'woocommerce_order_has_status', array( __CLASS__, 'deposit_pending_status' ), 10, 3 );
		}

	}

	/**
	 * Hook into WC_Order::has_status to make 'pending-deposit' equivalent to 'pending', since it's used as a check
	 * in the template for WC_Email_Customer_Invoice, i.e. it will only send pay links when the order status is 'pending'.
	 * @since 1.3.1
	 * @version 1.3.2
	 *
	 * @param bool $retval
	 ...
	 * @return bool
	 */
	public static function deposit_pending_status( $retval, $order, $status ) {
		return 'pending' === $status && 'pending-deposit' === $order->get_status() ? true : $retval;
	}
