...
class WC_Deposits_Cart_Manager {
	...
	public function get_deposit_remaining_amount() {
		...
		foreach ( WC()->cart->get_cart() as $cart_item ) {
			if ( ! empty( $cart_item['is_deposit'] ) && empty( $cart_item['payment_plan'] ) ) {
				...
				$deposit_amount = apply_filters( 'woocommerce_product_get_price', $cart_item['deposit_amount'], $_product );

				if ( isset( $cart_item['full_amount'] ) ) {
					if ( 'excl' === WC()->cart->tax_display_cart ) {
						$credit_amount += $this->get_price_excluding_tax( $_product, array( 'qty' => $quantity, 'price' => ( $full_amount - $deposit_amount ) ) );
					} else {
						$credit_amount += $this->get_price_including_tax( $_product, array( 'qty' => $quantity, 'price' => ( $full_amount - $deposit_amount ) ) );
					}
				}
			}
		}
	...
	public function display_cart_totals_after() {
		...
		<tr class="order-total">
			...
			<td data-title="<?php esc_attr_e( 'Due Today', 'woocommerce-deposits' ); ?>"><?php wc_cart_totals_order_total_html(); ?></td>
		</tr>
		<?php
		if ( $deferred_discount_amount > 0 ) {
			?>
			<tr class="order-total">
				...
				<td data-title="<?php esc_attr_e( 'Discount Applied Toward Future Payments', 'woocommerce-deposits' ); ?>"><?php echo wc_price( -$deferred_discount_amount ); ?></td>
			</tr>
			<?php
		}
		?>
	...
	public function add_order_item_meta_legacy( $item_id, $cart_item ) {
		if ( ! empty( $cart_item['is_deposit'] ) ) {
			...
			$deposit_amount = apply_filters( 'woocommerce_product_get_price', $cart_item['deposit_amount'], $cart_item['data'] );
