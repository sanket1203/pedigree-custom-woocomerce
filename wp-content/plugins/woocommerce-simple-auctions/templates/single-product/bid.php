		...
		<form class="auction_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo $product_id; ?>">
			...
			<?php if($product->get_auction_type() == 'reverse' ) : ?>
				<div class="quantity buttons_added">
					...
					<input type="number" name="bid_value" data-auction-id="<?php echo esc_attr( $product_id ); ?>"  <?php if ($product->get_auction_sealed() != 'yes'){ ?> value="<?php echo $product->bid_value() ?>" max="<?php echo $product->bid_value()  ?>"  <?php } ?> step="<?php echo ($product->auction_bid_increment) ? $product->auction_bid_increment : '1' ?>" size="<?php echo strlen($product->get_curent_bid())+2 ?>" title="bid"  class="input-text  qty bid text left">
			...
			<?php else : ?>	
				<div class="quantity buttons_added">
				 	<input type="button" value="+" class="plus" />		 	
					<input type="number" name="bid_value" data-auction-id="<?php echo esc_attr( $product_id ); ?>" <?php if ($product->get_auction_sealed() != 'yes'){ ?>  value="<?php echo $product->bid_value()  ?>" min="<?php echo $product->bid_value()  ?>" <?php } ?>  step="<?php echo ($product->auction_bid_increment) ? $product->auction_bid_increment : '1' ?>" size="<?php echo strlen($product->get_curent_bid())+2 ?>" title="bid"  class="input-text qty  bid text left">
					<input type="button" value="-" class="minus" />
				</div>	
		 	<button type="submit" class="bid_button button alt"><?php echo apply_filters('bid_text', __( 'Bid', 'wc_simple_auctions' ), $product); ?></button>
		 	<?php endif; ?>
