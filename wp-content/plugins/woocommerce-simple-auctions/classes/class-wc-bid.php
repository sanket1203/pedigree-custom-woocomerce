...
class WC_Bid {
	...
	public function placebid($product_id, $bid) {
		...
		if ((apply_filters( 'woocommerce_simple_auctions_before_place_bid_filter',$product_data) == false) OR !is_object($product_data) ){
			return false;
		}
		...
		do_action('woocommerce_simple_auctions_place_bid', array( 'product_id' => $product_id, 'is_proxy_bid' => $is_proxy_bid ));
